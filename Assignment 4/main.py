...
def add_to_cart(
    ...
    if product_id is None:
        raise HTTPException(status_code=400, detail="Missing product_id query parameter")
    ...
    if quantity < 1:
        ...
        raise HTTPException(status_code=400,detail=f"quantity must be atleast 1")
...
def checkout(checkout_data: CheckoutRequest, response: Response):
    ...
    if checkout_data is None:
        raise HTTPException(status_code=400, detail=f"request body is missing") 
